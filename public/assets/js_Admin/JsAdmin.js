...
//for upload cover img and name gallery
         $(".preload").css('display','block');
         var gallery_name=$('input[name="gallery_name"]').val();
         var tocken=$('input[name="_token"]').val();
...
//                          $(".print-error-msg").find("ul").html('');
                     ...
                     if(response.responseJSON.status=="fail"){
                          $('.msg').empty();
                          $(".print-error-msg").css('display','none');
//                          if(response.responseJSON.gallery_name !=""){
//                              $('.msg').append("<div class=\"alert alert-danger text-center\">please write the gallery name</div>");
//                          }
...
//                            if there is no path(no images)
                        ...
                        $.each(data.gallery_Images_path, function(index,value) {        
                            ...
                            var id=value.id;
                            var desc_text=[];
                         // desc_text.push($('#'+id+"text_desc").val());
                             $('#image_block').append('<div id="' + id + '"  style="float: left;display:block;" class="col-md-4">\n\
                                                        <div id="images"><img class ="img_src"   width= "182px" height="106px"  src='+"../"+"../"+"../"+path_image+' style="padding-right: 23px;padding-top: 16px;"/></div> \n\
                                                        <div id="check" ><input id="checked" class="source" type="checkbox"  value="' + id + '" name="check" style="margin-top: 6px;margin-left: 6px;width: 15px;height: 15px;"/></div>\n\
                                                        <form action="../../gallery/add_desc" method="get" name="add_desc" enctype="multipart/form-data" >\n\
                                                        <textarea id="' + id +"text_desc"+ '"  type="text"  placeholder="entre description"></textarea>\n\
                                                        <input type="hidden" name="_token" value="{{ Session::token() }}" id="desc_tocken">\n\
                                                        <button class="btn " type="submit"   id="add_description" style="position: absolute;top: 155px;right: 16px; background-color: #6cb6da;"><i class="fa fa-paper-plane" aria-hidden="true"></i></button></form>    \n\
                                                        </div>');      
...
//****************************ADD description in fgallery images********************************************************
$("body").on("click","#add_description",function(e){   
    e.preventDefault(); 
     $(".preload").css('display','block');
     $("#image_block").css('display','none');
     $(".msg").css('display','none');
    var tocken=$('#desc_tocken').val();
          ...
          if($(".source").is(':checked'))
           {
               ...
               var last_id= checkbox_value[checkbox_value.length-1];
//               alert(last_id);
               desc.push($('#'+last_id+"text_desc").val()); 
//               var last_desc= desc[desc.length-1];
//               all ids
     ...
     $.ajax({
            ...
            success:function(data) {    
               ...
               if(data.status=="sucess"){
                        ...
                        $('.msg').empty();
                        $(".print-error-msg").css('display','none');
                        $('.msg').append("<div class=\"alert alert-success text-center\">Add description  done succussfuly "+" </div>");

                    }  
                     if(data.status=="fail"){
                        $('.msg').empty();
                        $(".print-error-msg").css('display','none');
                        $('.msg').append("<div class=\"alert alert-danger text-center\">Fail! in Add description "+" </div>");
             ...
             error: function(data){
              console.log("error");
               $('.msg').empty();
                        $(".print-error-msg").css('display','none');
                        $('.msg').append("<div class=\"alert alert-danger text-center\">Fail! you must make check at lest one "+" </div>");
                    $(".preload").css('display','none');
                    $("#image_block").css('display','block');
                    $(".msg").css('display','block');
               }
...
$("body").on("click","#gallery_upload",function(e){
    ...
    $(".msg").css('display','none');
     ...
     $(this).parents("form").ajaxForm({
       ...
       complete: function(response) 
    {   
//alert(response.responseJSON.path);
...
//         show images
               ...
               $.each(response.responseJSON.path, function(index,value) {        
                    ...
                    var id=value.id;
                     ...
                     $('#image_block').append('<div id="' + id + '"  style="float: left;display:block;" class="col-md-4">\n\
                                              <div id="images"><img class="img_src" width= "225px"; height="128px" src='+"../"+"../"+"../"+path_image+' style="padding-right: 23px;padding-top: 16px;"/></div> \n\
                                              <div id="check" ><input id="checked" class="source" type="checkbox"  value="' + id + '" name="check" style="margin-top: 6px;margin-left: 6px;width: 15px;height: 15px;"/></div>\n\
                                                 <form action="../../gallery/add_desc" method="get" name="add_desc" enctype="multipart/form-data" >\n\
                                                        <input type="hidden" vlaue="' + id + '" name="text_id"  id="text_id"/>                                                        \n\
                                                        <textarea id="' + id +"text_desc"+ '"  type="text" value="" placeholder="entre description"></textarea>\n\
                                                        <input type="hidden" name="_token" value="{{ Session::token() }}" id="desc_tocken">\n\
                                                        <button class="btn " type="submit"   id="add_description" style="position: absolute;top: 155px;right: 16px; background-color: #6cb6da;"><i class="fa fa-paper-plane" aria-hidden="true"></i></button></form>    \n\
                                                        </div>');      
                     });
...
//             console.log(data);
                if(data){
                  ...
                  $.each(data.gallery_Images, function(index,value) {        
                    var path_image=value.path_image; 
                    var id=value.id;
                       $('#image_block').append('<div class="oneImage col-md-4" style="float: left;" ><div id="images"><img class="img_src"  width= "182px"; height="106px" src='+"../"+"../"+"../"+path_image+' style="padding-right: 23px;padding-top: 16px;"/></div> <div id="check" ><input id="checked" class="source" type="checkbox"  value="' + id + '" name="check" style="margin-top: 6px;margin-left: 6px;width: 15px;height: 15px;"/></div></div>');                             
                  });
                   $(".preload").css('display','none');     
                   $("#gallery_items_upload").css('display','block');
                   $("#image_block").css('display','block');
...
//                alert(checkbox);
                 ...
                 class_img='img_src'+checkbox;
//                alert(class_img);
                
       path=$("."+class_img).attr('src');
//       alert(path);
            });
        }
        if(!$(".check_gallery").is(':checked'))
