...
<div class="">
    ...
    <div class="row">
        ...
        <form method="post" action="{{ route('dashboard.send_notifications.inner.index') }}" class="form-horizontal form-label-left" enctype="multipart/form-data">
              {{ csrf_field() }}
              ...
              <div class="form-group row">
                <div class="col-md-12">
                  <div class="col-lg-12 col-md-12 col-sm-12">
                    <div class="col-lg-2">{{ $query['title'] }}</div>

                    @if( $query['alias'] == 'UsersSubscribedButNotTestedEver')
                          <div class="col-lg-2">
                            <input class="form-control" type="date" name="from_date_users_subscribed_but_not_tested_ever" value="{{ old('from_date_users_subscribed_but_not_tested_ever') }}">
                          </div>
                          <div class="col-lg-2">
                            <input class="form-control" type="date" name="to_date_users_subscribed_but_not_tested_ever" value="{{ old('to_date_users_subscribed_but_not_tested_ever') }}">
                          </div>
                          <div class="col-lg-2">
                            <select name="site_id_users_subscribed_but_not_tested_ever" class="form-control">
                              @foreach ($sites as $siteRow)
                                <option @if( old('site_id_users_subscribed_but_not_tested_ever') == $siteRow->id ) 'selected' @endif value="{{ $siteRow->id }}" >{{ $siteRow->title }}</option>
                              @endforeach
                            </select>
                            <input type="hidden" name="site_name_users_subscribed_but_not_tested_ever" value="{{ $siteRow->title }}"/>
                          </div>
                    @endif
                    ...
                    @if( $query['alias'] == 'UsersHasXCoursesToFinishDeiplom')
                          <div class="col-lg-2">
                            <select name="site_id" class="form-control">
                              @foreach ($sites as $siteRow)
                                <option @if( old('site_id') == $siteRow->id ) 'selected' @endif value="{{ $siteRow->id }}" >{{ $siteRow->title }}</option>
                              @endforeach
                            </select>
                            <input type="hidden" name="site_name_users_has_x_courses_to_finish_deiplom" value="{{ $siteRow->title }}"/>
                          </div>
                          <div class="col-lg-2">
                            <input class="form-control" type="number" name="more_than_x_courses" value="{{ old('more_than_x_courses') }}" placeholder="عدد الدورات الباقية">
                          </div>
                    @endif
                    ...
                    @if( $query['alias'] == 'UsersRegisterdFromTo')
                          <div class="col-lg-2">
                            <input class="form-control" type="date" name="from_date_users_registerd_from_to" value="{{ old('from_date_users_registerd_from_to') }}">
                          </div>
                          <div class="col-lg-2">
                            <input class="form-control" type="date" name="to_date_users_registerd_from_to" value="{{ old('to_date_users_registerd_from_to') }}">
                          </div>
                    @endif
                    ...
                    @if( $query['alias'] == 'UsersDidntTestedFromTo')
                          {{--
                            <div class="col-lg-2">
                              <input class="form-control" type="date" name="from_date_users_didnt_tested_from_to" value="{{ old('from_date_users_didnt_tested_from_to') }}">
                            </div>
                            <div class="col-lg-2">
                              <input class="form-control" type="date" name="to_date_users_didnt_tested_from_to" value="{{ old('to_date_users_didnt_tested_from_to') }}">
                            </div>
                          --}}
                    ...
                    @if( $query['alias'] == 'SendEmail')
                          <div class="col-lg-6">
                            <textarea class="form-control" name="ids" rows="8" cols="50">{{ old('ids') }}</textarea>
                          </div>
                    @endif
                    ...
                    <div class="col-lg-2">
                      <button type="submit" name="send_notifications_query" value="{{$query['alias']}}" class="btn btn-success">ارسال اشعارات</button>
                    </div>
              ...
              <div class="form-group row">
                ...
                <div class=" col-lg-6 col-md-9 col-sm-12">
                  عنوان الرسالة للطلاب<input type="text" class="form-control" name="title" required maxlength="100"/>
                  <br>
                </div>
