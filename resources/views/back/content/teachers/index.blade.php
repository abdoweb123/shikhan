        ...
        <tbody>
          @foreach ($data as $item)
                <tr id="{{ $item->id }}">
                    ...
                    <td>{{ $item->id }}</td>
                    <td>
                      <a href="{{ route('dashboard.teachers.edit' , [ 'id' => $item->id ] )}}" class="td_clickable">
                        {{ $item->name }}
                      </a>&nbsp;&nbsp;
                    </td>
                    ...
                    <td style="display: flex;">
                      @foreach (getActiveLanguages() as $language)
                        <a href="{{ route('dashboard.teachers.edit', ['id' => $item->id, 'language' => $language->alies ] )}}" class="btn btn-sm btn-warning">
                          {{ $language->alies }}
                        </a>
                      @endforeach
                    </td>
                    <td>
                      <div id="action_div" class="dt_action_div">
                        ...
                        <div><x-buttons.but_delete_inline link="{{ route('dashboard.teachers.destroy' ) }}" ids="{{ $item->id }}" icon='true'/></div>
                      </div>
                    </td>
                </tr>
          @endforeach
