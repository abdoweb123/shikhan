                    ...
                    <div class="col-lg-7">
                        <div class="courses-details-image text-center">
                          @if($diff_end_in_days <= 0 && $course->date_at != null)
                            @if (Auth::guard('web')->user())
                              @if( $lessons->first() != null && Auth::guard('web')->user()->courses()->find($course->id))
                                @foreach($lessons->get() as $kay_lesson => $lesson)
                                  @if($lesson->video != null)
                                      <label class="lesson_titel">{{ __('words.lesson_video')}} : </label>
                        ...
                        <div class="courses-details-desc">
                          @if (Auth::guard('web')->user())
                            @if($diff_end_in_days <= 0 && $course->date_at != null && $lessons->first() != null && Auth::guard('web')->user()->courses()->find($course->id) )

                                @foreach($lessons->get() as $kay_lesson => $lesson)
                                    @if($lesson->sound != null)
                                    <label class="lesson_titel">{{ __('words.lesson_sound')}} : </label>

                                          @if ( strpos($lesson->sound, 'google') ) {{-- google path --}}
                                              @if (strpos($lesson->sound, 'id='))
                                ...
                                @endif
                                @if($pdf != null )
                                  <label class="lesson_titel">{{ __('words.lesson_pdf')}} : </label>
                                    <iframe src="{{  $pdf }}" width="100%" height="500px"></iframe>
                                @endif

                          @else

                              @if($pdf != null && Auth::guard('web')->user()->courses()->find($course->id) )
                              <label class="lesson_titel">{{ __('words.lesson_pdf')}} : </label>
                                                ...
                                                <li class="accordion-item">
                                                    ...
                                                    <div class="accordion-content show">
                                                        <ul class="courses-lessons">
                                                          @foreach(App\course::where('site_id',$site->id)->where('id','!=',$course->id)->get() as $loopcourse)
