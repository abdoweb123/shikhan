              ...
              @if ($site->user_sucess)
                @if ($site->less_than_70)
                  <div class="row text-center prim-color-border" style="font-size: 20px;font-weight: bold;padding-top: 15px;box-shadow: 1px 6px 20px #0000001f;border-radius: 12px;padding-bottom: 12px;margin-top: 15px;">
                    ...
                    <div class="col-lg-3 text-center" style="color: #22a45b;"> {{ __('trans.rating')}} : {{ __('trans.rate.'.$site->user_site_rate) }}</div>


                      <div class="col-lg-4 text-center">
                        <span style="font-size: 18px;font-weight: normal;">{{ __('trans.diploma_cirt') }}</span>
                        <a data-href="{{ route('site-certificate-show', ['id' => $site->id, 'type' => 'jpg' ]) }}"
                        ...
                        <a data-href="{{ route('site-certificate-show', ['id' => $site->id, 'type' => 'pdf' ]) }}"
                          class="download_image btn but-default">
              ...
              <div class="col-11 card prim-color-border" style="margin: 10px;border-radius: 13px;">
                ...
                <div class="row card-body">
                  <div class="col-lg-6 col-sm-12 text-left">
                    ...
                    <div style="padding-top: 15px;"><span> {{ __('field.rate') }} : </span><span>{{ $course->id ? $course->course_rate : '' }}</span></div><!-- $course->id means user tested this course else not tested -->
                    <div style="padding-top: 7px;"><span> {{ __('field.degree') }} : </span><span>{{ $course->degree ? ($course->id ? round($course->degree,2) : '') : '' }}</span></div>

                    @if($course->id) <!-- user tested this course -->
                          @if($course->less_than_70)

                              <!-- download buttons -->
                              <div style="display: flex;text-align: right;padding-top: 7px;">
                                  &nbsp;{{ __('trans.download') }}&nbsp; <div class="loading_div_{{$course->id.'-'.$course->site_id}}" style="padding: 0px 4px;margin: 0px 4px;"></div>
                                  <a data-href="{{ route('download-certificate', ['id' => $course->id.'-'.$course->site_id, 'type' => 'jpg']) }}" data-id="{{ $course->id.'-'.$course->site_id }}"
                                    class="download_image btn but-default">
                                    <i class="fa fa-images" style="font-size: 13px;padding-left: 1px;padding-right: 1px;">&nbsp;{{ __('trans.image') }}&nbsp;</i>
                                  </a>
                                  <a data-href="{{ route('download-certificate', ['id' => $course->id.'-'.$course->site_id, 'type' => 'pdf']) }}" data-id="{{ $course->id.'-'.$course->site_id }}"
                                    class="download_image btn but-default">
                                    <i class="fa fa-images" style="font-size: 13px;padding-left: 1px;padding-right: 1px;">&nbsp;Pdf&nbsp;</i>
                                  </a>
                              </div>
                              ...
                              @if($course->userHasEjazaCertificate)
                                <a data-href="{{ route('download-ejaza-certificate-show', ['id' => $course->id.'-'.$course->site_id, 'type' => 'jpg']) }}"
                                  class="download_image btn but-default">
                                  <i class="fa fa-images" style="font-size: 13px;padding-left: 1px;padding-right: 1px;">&nbsp; الاجازة صورة&nbsp;</i>
                                </a>
                                <a data-href="{{ route('download-ejaza-certificate-show', ['id' => $course->id.'-'.$course->site_id, 'type' => 'pdf']) }}"
                                  class="download_image btn but-default">
