                  ...
                  <div class="col-lg-4">
                    <div class="col-lg-12" style="padding: 8px 8px;margin: 8px 0px;border-radius: 11px;border: 1px solid #e6e6e6;
                      ...
                      <div class="row">
                        ...
                        <!-- image -->
                        <div class="col-lg-3" >
                            <!-- <img class="img-fluid" onclick='getMyCourses({{ $term->id }})' data-toggle="modal" data-target=".bd-example-modal-lg" src="{{ url($term->logo_path) }}" style="border-radius: 7px;cursor: pointer;"> -->
{{--                            <a href="{{ route('courses_certificates', ['site_id'=>$term->id ] ) }}"><img class="img-fluid" data-toggle="modal" data-target=".bd-example-modal-lg" src="{{ url($term->ImageDetailsPath) }}" style="border-radius: 7px;cursor: pointer;"></a>--}}
                        </div>

                        <!-- title -->
{{--                        <div class="col-lg-9" onclick='getMyCourses({{ $term->id }})' data-toggle="modal" data-target=".bd-example-modal-lg" style="font-size: 17px;font-weight: bold;color: #825932;cursor: pointer;">{{ $term->title }}</div>--}}
                        <div class="col-lg-9" style="font-size: 17px;font-weight: bold;color: #825932;cursor: pointer;">{{ $term->title }}</div>
                        <div class="col-lg-9" style="font-size: 17px;font-weight: bold;color: #825932;cursor: pointer;">
{{--                            <a href="{{ route('courses_certificates', ['site_id'=>$term->id ] ) }}">{{ $term->name }}</a>--}}
                        </div>

                        <!-- finished courses -->
                        <div class="col-lg-6 text-center" style="font-size: 16px;font-weight: bold;color: #825932;padding-top: 15px;"><span class="prim-color">{{ __('trans.achived')}} : {{ $term->course_term_finished_count }} {{ __('trans.course')}}</span><span  style="color: gray;"> / {{ $term->courses_count }}</span></div>

                        <!-- courses results details -->
                        <div class="col-lg-6 text-center" style="padding: 10px 0px;">
                          <!-- <button class="btn btn-outline-primary" onclick='getMyCourses({{ $term->id }})' data-toggle="modal" data-target=".bd-example-modal-lg" style="cursor: pointer;">تفاصيل النتائج</button> -->
{{--                          <a href="{{ route('courses_certificates', ['site_id'=>$term->id ] ) }}" class="btn but-default-rounded">{{ __('meta.title.courses_cirts')}}</a>--}}
                          <a href="{{ route('courses_certificates_term', ['term_id'=>$term->id ] ) }}" class="btn but-default-rounded">{{ __('meta.title.courses_cirts')}}</a>
                        </div>

                        @isset($term->term_results)
                            @if ($term->term_results->rate > 0)
{{--                                <div class="col-lg-6 text-center" style="font-size: 16px;font-weight: bold;color: #825932;padding-top: 15px;"><span  class="prim-color">{{ __('trans.rating') }} : {{ __(getRate($term->term_results)) }}</span></div>--}}
                                <div class="col-lg-6 text-center" style="font-size: 16px;font-weight: bold;color: #825932;padding-top: 15px;"><span  class="prim-color">{{ __('trans.rating') }} : {{ __('trans.rate.'.$term->term_results->rate) }}</span></div>
                                <div class="col-lg-6 text-center" style="padding: 10px 0px;"><button class="but-more" data-toggle="modal" data-target="#cirt_modal_{{ $term->id }}" style="cursor: pointer;">{{ __('trans.download_term_cirt') }}</button></div>
                            @endif
                        @endisset
                      </div>

                      @include('front.include.site_progress', [ 'courses_count' => $term->courses_count, 'finished_courses_count' => $term->finished_courses_count ])
                  ...
                  @isset($term->term_results)
                      @if ($term->term_results->rate > 0)
                            <div id="cirt_modal_{{ $term->id }}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                                ...
                                <div class="modal-content" style="text-align: center;">
                                    ...
                                    <div class="modal-body">
                                      <div class="col-lg-12" style="padding: 8px 0px;margin: 8px 0px;">
                                                ...
                                                <div class="col-lg-12" id="error_div_{{$term->id}}" style="color: red;"></div>


                                                <div class="col-lg-12" style="padding: 8px 0px;margin: 8px 0px;">
                                                  <span style="font-size: 19px;font-weight: normal;">{{ __('trans.download_cirt_term') }}</span>
                                                  <a data-href="{{ route('term-certificate-show', ['id' => $term->id, 'type' => 'jpg' ]) }}" data-error-div="error_div_{{$term->id}}"
                                                    class="download_image btn btn-outline-success">
                                                    <i class="fa fa-images" style="font-size: 13px;padding-left: 1px;padding-right: 1px;">&nbsp;{{ __('trans.image') }}&nbsp;</i>
                                                  </a>
                                                  <a data-href="{{ route('term-certificate-show', ['id' => $term->id, 'type' => 'pdf' ]) }}" data-error-div="error_div_{{$term->id}}"
                                                    class="download_image btn btn-outline-success">
                                                ...
                                                <!-- extra certificates -->
                                                @foreach ($term->extra_certificates ?? [] as $siteExtraCertificat)
                                                  @if ($siteExtraCertificat->result['deserve'])
                                                    <div class="col-lg-12" style="padding: 8px 0px;margin: 8px 0px;">
                                                      ...
                                                      <a data-href="{{ route('download-site-extra-certificate-show', ['id' => $term->id, 'extra_certificate_id' => $siteExtraCertificat->id, 'type' => 'jpg' ]) }}"
                                                        class="download_image btn btn-outline-success">
                                                        <i class="fa fa-images" style="font-size: 13px;padding-left: 1px;padding-right: 1px;">&nbsp;صورة&nbsp;</i>
                                                      </a>
                                                      <a data-href="{{ route('download-site-extra-certificate-show', ['id' => $term->id, 'extra_certificate_id' => $siteExtraCertificat->id, 'type' => 'pdf' ]) }}"
                                                        class="download_image btn btn-outline-success">
