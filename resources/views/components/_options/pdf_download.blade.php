...
<a target="_blank" href="{{ $pdf }}/view" download style="display: flex;font-size: 27px;padding: 15px 0px;">
  <img src="
    @if($item->options->icon != null) {{asset($item->options->icon)}}
    @else {{asset('images/default.jpg')}} @endif
    " width="50" style="margin: 10px;" title="{{ $item->options->option_info->first()?->title }}" >
    <span>{{-- $item->options->option_info->first()->title --}} {{ $item->title }}</span>
</a>
