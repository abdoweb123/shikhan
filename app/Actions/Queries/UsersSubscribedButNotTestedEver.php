...
// طلاب اشتركو فى دورات من تاريخ معين ولم يختبروا فيها
class UsersSubscribedButNotTestedEver
{
        ...
        return DB::Table('members')
            ...
            ->leftjoin('course_tests_results', function ($join) {
                  ...
                  ->when(! empty($this->site_id), function($q){
                      ...
                      return $q->where('course_tests_results.site_id', $this->site_id);
                  });
            })
            ->when(! empty($this->site_id), function($q){
                if (is_array($this->site_id)){
                  return $q->whereIn('site_subscriptions.site_id', $this->site_id);
                }
                return $q->where('site_subscriptions.site_id', $this->site_id);
            })
            ->where('site_subscriptions.created_at', '>=', date('Y-m-d', strtotime($this->from_date)) )
            ->where('site_subscriptions.created_at', '<=', date('Y-m-d', strtotime($this->to_date)) )
