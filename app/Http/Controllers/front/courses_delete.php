...
class courses extends Controller
{
        ...
        $data['result'] = $data['site']->terms()->where('status', 1)->whereTranslation('locale', app()->getlocale())
            ->with(['courses' => function($q){
              return $q->where('status', 1)->whereTranslation('locale', app()->getlocale())
                ->select('courses.id','courses.title','logo','exam_at','exam_approved','courses.new_flag')
                ->orderBy( DB::raw('ISNULL(sort), sort'), 'ASC' );
        }])->get();



        $siteCoursesCount = $data['result']->count();
    ...
    public function show(Request $request)
    {
        ...
        $data['lessons'] = LessonOld::where('course_id', $data['course']->id)->where(['is_active'=> 1])
    ...
    public function getSuccessedUsersOfCountryOfSite(Request $request)
    {
        ...
        if($country){
          $data['currentCountry'] = App\Country::where(['status' => 1,'arabic' => $country])->firstOrFail();
        }
    ...
    public function post(Request $request)
    {
        ...
        $data['post'] = LessonOld::where(['is_active'=> 1])->where('course_id',$data['course']->id)->whereTranslation('alias',$post_alias)->whereTranslation('trans_status', 1)->first();
        if(!$data['post']){
       ...
       if(! Auth::guard('web')->user()->courses()->find($data['course']->id)){
        ...
        // {
        //      $post = LessonOld::where(['is_active'=> 1])->whereTranslation('alias',$post_alias)->whereTranslation('is_active', 1)->firstOrFail();

        //     Cache::forever($cach_name,$post);
        // }
        $data['posts'] =  LessonOld::where(['is_active'=> 1])->whereIn('id',$data['course']->post_ids)->whereTranslation('trans_status', 1)->get();
